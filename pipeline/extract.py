...
class Extract(luigi.Task):
        ...
        try:
            ...
            for index, table_name in enumerate(self.tables_to_extract):
                try:
                    ...
                    df = pd.read_sql_query(extract_query.format(
                        table_name=table_name), src_engine)
            ...
            logging.error(f"Extract All Tables From Sources - FAILED")
    ...
    def output(self):
        ...
        for table_name in self.tables_to_extract:
            outputs.append(luigi.LocalTarget(
                f'{DIR_TEMP_DATA}/{table_name}.csv'))

        outputs.append(luigi.LocalTarget(
            f'{DIR_TEMP_DATA}/extract-summary.csv'))
